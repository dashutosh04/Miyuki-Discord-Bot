  ...
  async execute(client, message, args) {
    ...
    if (!emoji && emoji_regex.test(args[0])) {
      const bada = new MessageEmbed()
        .setImage(
          `https://emojiapi.dev/api/v1/${emojiUnicode(args[0])}/128.png`
        )
        .setColor(`#2c2f33`);
      return message.reply({ embeds: [bada] });
    }
    ...
    const tada = new MessageEmbed().setImage(Emojilink).setColor(`#2c2f33`);
    message.reply({ embeds: [tada] });
  },
};
