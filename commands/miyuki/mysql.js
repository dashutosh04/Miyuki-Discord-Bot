...
async execute(client,message,args) {
    ...
    client.connection.query(`${args.join(' ')}`, function (error, results, fields) {
        ...
        console.log(results)
        if(results.length == 0) return message.channel.send(`${client.emotes.warning} **|** No results .`)
        
        if (results) return message.channel.send(`\`\`\`${JSON.stringify(results).slice(0,998)}\`\`\``);
