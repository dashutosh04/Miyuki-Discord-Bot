  ...
  async execute(client, message, args) {
    ...
    if (!args[0]) {
      const actions = message.client.commands
        .filter((x) => x.category == "Actions")
        .map((x) => "`" + x.name + "`")
        .join(" ,  ");
      const animals = message.client.commands
        .filter((x) => x.category == "Animals")
        .map((x) => "`" + x.name + "`")
        .join(" ,  ");
      const emotes = message.client.commands
        .filter((x) => x.category == "Emotes")
        .map((x) => "`" + x.name + "`")
        .join(" ,  ");
      const Mg = message.client.commands
        .filter((x) => x.category == "Image Generator")
        .map((x) => "`" + x.name + "`")
        .join(" ,  ");
      const music = message.client.commands
        ...
        .map((x) => "`" + x.name + "`")
        .join(" ,  ");
      const Tg = message.client.commands
        .filter((x) => x.category == "Text Generator")
        .map((x) => "`" + x.name + "`")
        .join(",  ");
      const utility = message.client.commands
        .filter((x) => x.category == "Utility")
        .map((x) => "`" + x.name + "`")
        .join(" ,  ");
      const mod = message.client.commands
        .filter((x) => x.category == "Mod")
        .map((x) => "`" + x.name + "`")
        .join(" ,  ");
      ...
      const ts = new MessageEmbed()
        ...
        .addFields(
          ...
          {
            name: "> Utilisation",
            value: command.utilisation.replace("${prefix}", prefix),
            inline: true,
          }
        )
        .setTimestamp();
