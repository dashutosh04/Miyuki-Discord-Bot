...
execute: async (client, message, args) => {
    ...
    if(args[0].toLowerCase().trim() === 'me') return message.reply('You Poked yourself \nNext time mention someone else.')

    if(target){
        if(target.username === message.author.username) return message.reply('You Poked yourself \nNext time mention someone else.')
  ...
  let hug = new MessageEmbed()
    .setColor("RANDOM")
    .setFooter(`${message.author.username} Pokes ${user}`)
    .setTimestamp()
    .setImage(owo.url)
