...
async execute(client , message, args) {
    ...
    client.channels.cache.get(message.channel.id).messages.fetch({ limit: 2 }).then(messages => {var lastMessage = messages.first()
    lastMessage.react(emoji.then())
});
const BotReactions = message.reactions.cache.filter(reaction => reaction.users.cache.has('914092958800838728'));
